...
(unwrap wraped #{:article})
 ...
 (def employees-data (set (first (map #(read-string (str "[" % "]"))  (let [rdr (clojure.java.io/reader "resources/employees.clj")]
          (line-seq rdr))))))


 (time (def employees (rel [:emp_no :birth_date :first_name :last_name :gender :hire_date] employees-data))); auf MAC: 65.3 ms

 (def salaries-data (set (first (map #(read-string (str "[" % "]"))  (let [rdr (clojure.java.io/reader "resources/salaries.clj")]
          (line-seq rdr))))))
